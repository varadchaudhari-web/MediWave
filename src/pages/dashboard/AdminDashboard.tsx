...
export default function AdminDashboard() {
  ...
  const handleAddDoctor = () => {
    if (!newDoctor.name || !newDoctor.specialty) { toast.error('Name and specialty are required'); return; }
    addDoctor({
      name: newDoctor.name,
  ...
  const handleAddPatient = () => {
    if (!newPatient.name || !newPatient.email) { toast.error('Name and email are required'); return; }
    addPatient({
            ...
            <div className="flex items-center justify-between gap-4 flex-wrap">
              ...
              <div className="flex gap-3">
                <div className="flex items-center gap-2 bg-white border border-slate-200 rounded-xl px-3 py-2">
                  ...
                  <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="Search patients..." className="text-xs focus:outline-none w-36" />
                </div>
            ...
            <div className="grid md:grid-cols-2 gap-5">
              {[
                ...
                <div key={s.label} className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
                  ...
                  <input type={s.type} defaultValue={s.value} className="w-full px-3 py-2 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-sky-500" />
                </div>
              ))}
            </div>
              ...
              {[
                ...
                <div key={f.key}>
                  ...
                  <input type={f.type} placeholder={f.placeholder}
                    ...
                    onChange={e => setNewPatient(prev => ({ ...prev, [f.key]: e.target.value }))}
                    className="w-full px-4 py-2.5 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-sky-500" />
                </div>
              ))}
              <div className="flex gap-3 pt-2">
